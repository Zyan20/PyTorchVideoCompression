...
def calc_psnr(ref: np.ndarray, target: np.ndarray):
    """
    unnormalized RGB images
    """
...
def calc_msssim(ref: np.ndarray, target: np.ndarray):
    tmpssim = msssim(target[:, :, 0], ref[:, :, 0])
    tmpssim += msssim(target[:, :, 1], ref[:, :, 1])
    tmpssim += msssim(target[:, :, 2], ref[:, :, 2])
